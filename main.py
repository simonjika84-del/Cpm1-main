...
class T:
    ...
    def dashboard(record, email, uid):
        name   = str(record.get("Name","Unknown")).strip()[:28]
        pid    = str(record.get("localID","—")).strip()[:18]
        ...
        return (
            f"╭━━━━━━━━━━━━━━━━━━━━━━━━━━━━╮\n"
            f"│  👑 <b>DASBOARD BOT MASKYY</b>{badge}\n"
            ...
            f"│  💰 Money        <b>${money:,}</b>\n"
            f"│  🪙 Coins        <b>{coin:,}</b>\n"
            f"│  🏆 Wins/Losses  <b>{wins:,}W / {loses:,}L</b>\n"
            f"│  🎮 Levels Done  <b>{done}</b>\n"
            f"│  🛞 Wheels       <b>{len(wheels)}</b>\n"
            f"│  🎭 Animations   <b>{len(anims)}</b>\n"
            f"│  👥 Friends      <b>{friends}</b>\n"
            f"╰────────────────────────────────────╯\n\n"
            f"────────── ⚡ <b>QUICK MENU</b> ⚡ ──────────\n"
...
class K:
    ...
    def feat():
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚗  W16 ENGINE", callback_data="f_w16"),
             InlineKeyboardButton(text="📣  HORNS", callback_data="f_horns")],
            [InlineKeyboardButton(text="🛡  NO DAMAGE", callback_data="f_damage"),
             InlineKeyboardButton(text="⛽  UNLIMITED FUEL", callback_data="f_fuel")],
            [InlineKeyboardButton(text="💨  SMOKE", callback_data="f_smoke"),
             InlineKeyboardButton(text="🎭  ANIMATIONS", callback_data="f_anims")],
            [InlineKeyboardButton(text="🛞  WHEELS", callback_data="f_wheels"),
             InlineKeyboardButton(text="🏠  HOUSES", callback_data="f_houses")],
            [InlineKeyboardButton(text="🎮  ALL LEVELS", callback_data="f_levels"),
             InlineKeyboardButton(text="🏅  MAX RANK", callback_data="f_rank")],
            [InlineKeyboardButton(text="🚀  ✦ UNLOCK ALL FEATURES ✦", callback_data="f_all")],
...
async def cb_feat_menu(cb: CallbackQuery):
    ...
    await cb.message.edit_text(txt, reply_markup=K.feat())
    await cb.answer()
...
async def cb_set_menu(cb: CallbackQuery):
    ...
    # Ambil data akun CPM yang sedang login agar nama + Player ID tampil otomatis.
    td = nuker.get_token_data(uid) or {}
    email = td.get("email", "—")
    record = nuker.get_record(uid, email) or {}
    cpm_name = str(record.get("Name", "Unknown")).strip()[:28]
    cpm_id = str(record.get("localID", "—")).strip()[:18]
    rank_value = str(record.get("Rank") or record.get("rank") or record.get("Rating") or "—").strip()[:24] or "—"

    settings_text = (
        f"{B}\\n"
        f"  🔧  <b>𝗦𝗘𝗧𝗧𝗜𝗡𝗚𝗦 𝗕𝗢𝗧 𝗠𝗔𝗦𝗞𝗬𝗬</b>\\n"
        f"  ✦ <i>Account Management Center</i>\\n"
        f"{B}\\n\\n"
        f"╭──────── 👤 <b>CPM ACCOUNT</b> ────────╮\\n"
        f"│\\n"
        f"│  📧 <b>Email CPM :</b> <code>{email}</code>\\n"
        f"│  👤 <b>Nama Akun CPM :</b> {cpm_name}\\n"
        f"│  🆔 <b>Player ID CPM :</b> <code>{cpm_id}</code>\\n"
        f"│  🏅 <b>Rank CPM :</b> {rank_value}\\n"
        f"│\\n"
        f"╰────────────────────────────────────╯\\n\\n"
        f"⚙️ <b>ACCOUNT CONTROL CENTER</b>\\n"
        f"├─ ✏️ Update account name\\n"
        f"├─ 🆔 Manage Player ID\\n"
        f"├─ 🏆 Update wins / losses\\n"
        f"└─ 🔧 Repair account data\\n\\n"
        f"<b>SELECT AN OPTION BELOW</b>\\n"
        f"Manage your CPM account settings using the options below."
    )
