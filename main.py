...
class T:
    ...
    @staticmethod
    def role_users():
        """Display users in exactly three mutually exclusive role groups."""
        ...
        labels = {
            ...
            "admin": ("🛡", "𝗨𝗦𝗘𝗥 𝗔𝗗𝗠𝗜𝗡", "Akses admin"),
        }

        txt = f"{B}\n  👥  𝗨𝗦𝗘𝗥 𝗥𝗢𝗟𝗘𝗦\n{B}\n\n"
        txt += "  🔹 Setiap user hanya ditampilkan pada 1 role.\n\n"

        for role in ("user", "vip", "admin"):
            icon, title, desc = labels[role]
            members = groups[role]
            txt += f"  {icon} <b>{title}</b> — {len(members)}\n"
            txt += f"  └ {desc}\n"
            ...
            for uid in members[:20]:
                ...
                name = info.get("name", f"User {uid}")[:18]
                username = info.get("username", "")
                suffix = f" @{username}" if username else ""
                txt += f"     • {name}{suffix} <code>{uid}</code>\n"

            if len(members) > 20:
                txt += f"     • ... +{len(members)-20} lainnya\n"
            txt += "\n"
    ...
    def banned_users(page=0, per_page=15):
        """Display banned users with stored Telegram profile data."""
        users = STORE.get("users", {})
        ...
        txt = f"{B}\\n  🚫  𝗕𝗔𝗡𝗡𝗘𝗗 𝗨𝗦𝗘𝗥𝗦\\n{B}\\n\\n"
        txt += f"  Total banned: <b>{total}</b>\\n"
        if total == 0:
            txt += "\\n  • Tidak ada user yang sedang diban."
            return txt

        txt += f"  Halaman {page + 1}/{pages}\\n\\n"
        for i, uid in enumerate(members, start=start + 1):
            info = users.get(str(uid), {})
            name = escape(str(info.get("name", f"User {uid}")))[:28]
            username = str(info.get("username", "")).strip()
            suffix = f" @{escape(username)}" if username else ""
            txt += f"  <b>{i}.</b> 👤 {name}{suffix}\\n"
            txt += f"      🆔 <code>{uid}</code>\\n\\n"
