...
class T:
    ...
    def welcome(name, username, uid):
        now = datetime.now()
        display_name = escape(str(name or "User").strip()[:28])
        display_username = escape(username or "N/A")
    ...
    def admin_panel(uid):
        ...
        return (
            f"{B}\n"
            f"👑 <b>ADMIN PANEL MAS KYY</b>\n"
            f"{B}\n\n"
            f"🛡 <b>Role</b>         {badge}\n"
            f"⚙️ <b>Maintenance</b>  {maint}\n\n"
            f"📊 <b>OVERVIEW</b>\n"
            f"├ 👥 Users      <code>{len(ALLOWED_USERS)}</code>\n"
            f"├ 💎 VIP        <code>{len(VIP_USERS)}</code>\n"
            f"├ ⏳ Pending    <code>{len(PENDING)}</code>\n"
            f"├ 🚫 Banned     <code>{len(BANNED)}</code>\n"
            f"└ 🛡 Admins     <code>{len(ADMINS)}</code>"
        )
    ...
    def stats():
        ...
        return (
            ...
            f"⚡ <b>BOT ACTIVITY</b>\n"
            f"├ ⚡ Actions     <code>{actions}</code>\n"
            f"├ 🔓 Unlocks    <code>{unlocks}</code>\n"
            f"└ 🔐 Logins     <code>{logins}</code>\n\n"
            f"📅 <b>TODAY</b>\n"
...
class K:
    ...
    def login():
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔐  Sign In  •  Secure Access", callback_data="login")],
        ])
    ...
    def admin(uid):
        ...
        if lvl >= 10:
            ...
            b.append([
                InlineKeyboardButton(text="➕ Add User", callback_data="a_adduser"),
            ...
            b.append([
                ...
                InlineKeyboardButton(text="ℹ️ Profile", callback_data="a_profile"),
            ])
